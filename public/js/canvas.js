...
function naopinte(e) {
  if (
    ...
    e.clientY > canvasRect.height + canvasRect.y ||
    e.clientX > canvasRect.width + canvasRect.x
  ) {
    ctx.beginPath();
...
function atualizarModo(modo) {
  ...
  if (
    modo.currentTarget.id == "linha" ||
    modo.currentTarget.id == "circulo" ||
    modo.currentTarget.id == "retangulo"
  ) {
    trocarCanvas();
  } else {
    destrocarCanvas();
...
function desenharRetangulo(e) {
  ...
  ctx2.rect(
    xInicial,
    yInicial,
    e.clientX - canvasRect.x - xInicial,
    e.clientY - canvasRect.y - yInicial
  );
  ctx2.stroke();
}
